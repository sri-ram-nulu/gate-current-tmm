...
This can be expressed in a matrix vector form in terms of forward-propagating component ($\psi_{i}^+$; incident) and the backward-propagating component ($\psi_{i}^-$; reflected): $$\mathbf{\Psi}(x) = \begin{pmatrix} \psi_{i}^+(x) \\ \psi_{i}^-(x) \end{pmatrix} = \begin{pmatrix} A_i e^{j k_i x} \\ B_i e^{-j k_i x} \end{pmatrix}$$
